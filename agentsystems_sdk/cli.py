...
# Treat the gateway host as configurable via env; default localhost when running dev compose
_GATEWAY_HOST = os.getenv("AGENT_GATEWAY", "http://localhost:18080").rstrip("/")

def _gw() -> str:
    """Return base URL of the Agent Control Plane gateway."""
    return _GATEWAY_HOST

registry_app = typer.Typer(help="Manage container registries")
app.add_typer(registry_app, name="registry")

@registry_app.command("list")
def registry_list() -> None:
    """List registries configured in the gateway catalogue."""
    try:
        r = httpx.get(f"{_gw()}/registries", timeout=10)
        r.raise_for_status()
    except Exception as e:
        typer.secho(f"Failed to contact gateway: {e}", fg=typer.colors.RED)
    ...
    for col in ("id", "name", "url", "auth_type", "enabled"):
        table.add_column(col)
    for row in rows:
        table.add_row(str(row["id"]), row["name"], row["url"], row["auth_type"], str(row["enabled"]))
    console.print(table)

@registry_app.command("add")
def registry_add(
    url: str = typer.Argument(..., help="Registry base URL, e.g. https://registry.agentsystems.ai"),
    name: str = typer.Option(None, "--name", help="Friendly name"),
    auth: str = typer.Option("none", "--auth", help="Auth type: none|basic|bearer"),
    username: str = typer.Option(None, "--username", "-u", help="Username (basic auth)"),
    password: str = typer.Option(None, "--password", "-p", help="Password or token (basic/bearer)"),
    enabled: bool = typer.Option(True, "--enabled/--disabled", help="Enable immediately or add disabled"),
) -> None:
    """Add a new registry entry to the gateway catalogue."""
    payload = {
    ...
    try:
        r = httpx.post(f"{_gw()}/registries", json=payload, timeout=10)
        r.raise_for_status()
    except Exception as e:
        typer.secho(f"Failed to add registry: {e}", fg=typer.colors.RED)
        raise typer.Exit(code=1)
    console.print(f"[green]✓ Added registry[/green] ID={r.json()['id']}")

@registry_app.command("toggle")
def registry_toggle(
    registry_id: str = typer.Argument(..., help="Registry UUID"),
    enable: bool = typer.Option(True, "--enable/--disable", help="Enable or disable the registry"),
) -> None:
    """Enable or disable a registry by ID."""
    payload = {"enabled": enable}
    try:
        r = httpx.patch(f"{_gw()}/registries/{registry_id}", json=payload, timeout=10)
        if r.status_code == 404:
            typer.secho("Registry not found", fg=typer.colors.RED)
            raise typer.Exit(code=1)
        r.raise_for_status()
    except Exception as e:
        typer.secho(f"Failed to update registry: {e}", fg=typer.colors.RED)
        raise typer.Exit(code=1)
    state = "enabled" if enable else "disabled"
    console.print(f"[green]✓ {state}[/green] {registry_id}")
...
def registry_delete(
    ...
    if not yes and not typer.confirm(f"Delete registry {registry_id}?"):
        raise typer.Exit()
    try:
        r = httpx.delete(f"{_gw()}/registries/{registry_id}", timeout=10)
        if r.status_code == 404:
            typer.secho("Registry not found", fg=typer.colors.RED)
            raise typer.Exit(code=1)
        r.raise_for_status()
    except Exception as e:
        typer.secho(f"Failed to delete registry: {e}", fg=typer.colors.RED)
        raise typer.Exit(code=1)
    console.print(f"[green]✓ deleted[/green] {registry_id}")
